...
?>
  ...
  <ul class="nav navbar-nav primary-nav">
    ...
    <li class="current-menu-item menu-item-has-children dropdown">
      ...
      <a href="javascript:mostrar(4)" class="dropdown-hover"> <span class="underline">Productos</span>  <span class="caret"></span>
...
<div id="wrapper6" style="display:none" class="wide-wrap ">
  <div class="offcanvas-overlay"></div>
  <div class="content-container">
    <div class="container">
      <div class="row">
                ...
                <table class="table shop_table cart">
                  <thead>
                    <tr>
                      <th class="product-remove hidden-xs">Acción</th>
                      <th class="product-name">Código</th>
                      <th class="product-price text-center">Tipo de Producto</th>
                      <th class="product-quantity text-center">Categoría</th>
                      <th class="product-subtotal text-center hidden-xs"></th>
                    </tr>
                  </thead>
                  <tbody>
...
$sql60 = "SELECT nombre_categoria FROM categoria WHERE idcategoria='$idcategoria' LIMIT 1";
  ...
  echo "<td class='product-name'>$fila60[nombre_categoria]
  <dl class='variation'> \n
  </td>\n";
      ...
      <div class="form-row">
       <a data-rel="loginModal5" href="#" class="btn btn-outline rounded">Agregar Producto</a>
     </div>
   </div>
 </div>
</div>
...
<div class="modal fade user-login-modal" id="userloginModal5" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <form name="formulario_usuario05" id="userloginModalForm">
        <div class="modal-header">
          <button id="modal-c" type="button" class="close" data-dismiss="modal"> <span aria-hidden="true">&times;</span><span class="sr-only">Cerrar</span>
          </button>
...
<div class="modal-body">
         <div class="form-group">
             <label>Seleccione Categoria</label>
             <select name="tipo" id="category" required>
             <option value="0">Seleccione ....</option>
             <?
              while ($query_result = $result2->fetch_array())
              {
                $idcategoria= $query_result['idcategoria'];
                $nombre_categoria= $query_result['nombre_categoria'];

                ?><option <? echo"value='$idcategoria'"; ?>><? echo"$nombre_categoria"; ?></option><?
              }
              ?>
              </select>
        </div>
          <div class="form-group">
            ...
            <input type="text" id="nombrep" name="nombrep" required class="form-control" value="" placeholder="Nombre del Usuario" onkeypress="return validar(event)" required="required">
          </div>


        <div id="grabar05" class="modal-footer">
          <button type="button" class="btn btn-default btn-outline" onclick="enviarHTTP05()">Grabar</button>
        </div>
        <div id="x105" style="display: none" class="modal-footer">
          <label>Producto ya existe</label>
        </div>
        <div id="x205" style="display: none" class="modal-footer">
          <label>El producto Se Registro Correctamente <div class="modal-footer">
          <button type="button" class="btn btn-default btn-outline" data-dismiss="modal" onclick="actualizar(4)">Continuar</button>
        </div></label>
        </div>
      </form>
